<section class="bg-dark px-4 md:px-8 lg:px-16">

  <div class="relative container flex flex-col md:flex-row bg-dark">
    <div class="w-full md:w-6/12 pr-4">
      <nav aria-label="Primary navigation">
        <ul role="list">
          <li class="text-xl lg:text-2xl text-light font-bodybold pb-2 mt-8 border-b">Isaac Rossi</li>
          <a href="#work"><li class="flex justify-between text-xl lg:text-2xl text-light font-body pb-2 pt-2 border-b">Work<span class="nav-arrow text-accent">↓</span></li></a>
          <a href="#about"><li class="flex justify-between text-xl lg:text-2xl text-light font-body pb-2 pt-2 border-b">About<span class="nav-arrow text-accent">↓</span></li></a>
          <a href="#blog"><li class="flex justify-between text-xl lg:text-2xl text-light font-body pb-2 pt-2 border-b">Blog<span class="nav-arrow text-accent">↓</span></li></a>
        </ul>
      </nav>
      <h1 class="hero-text absolute text-5xl md:text-6xl lg:text-7xl text-light text-center md:text-left leading-3 md:leading-2.5 w-full mx-auto uppercase font-heading tracking-wide absolute bottom-5 md:bottom-20 left-0">Frontend Developer</h1>
    </div>


    <div class="w-full md:w-6/12">
      <image class="w-full rounded-tr-4xl mt-8 pb-14" src="<?php echo get_template_directory_uri() . '/images/me.jpg'; ?>"></image>
    </div>
  </div>
 
</section>