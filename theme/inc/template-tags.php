	...
	function portfolio_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span>' . esc_html__( 'Posted in %1$s', 'portfolio' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span>' . esc_html__( 'Tagged %1$s', 'portfolio' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span> on %s</span>', 'portfolio' ),
						array(
							'span' => array(
								'class' => array(),
							),
